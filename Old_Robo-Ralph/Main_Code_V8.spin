...
'ACCEPTS DATA FROM PROCESSING. WILL TRACK A USER'S HAND. WILL NOT AVOID OBSTICLES YET.
...
'________________________________________________________THESE ARE ALL OF THE PIN DESIGNATIONS_____________________________________________________________________| 
...
'I'm not sure what pins 28 and 29 are for.                                                                                                                      |
'Pin 30 = tx
'Pin 31 = rx
...
repeat          'FIRST ADJUST X-POS THEN Z-POS.
    ...
    if (K_DataIn_x < -200) and (range0 > Ping_dist) AND (IR_range1 < IR_dist)              
       K_CW                               
       waitcnt(clkfreq/100 + cnt)
       
    if (K_DataIn_x > 200) and (range2 > Ping_dist) AND (IR_range2 < IR_dist) 
       K_CCW                              
       waitcnt(clkfreq/100 + cnt)
       
    if (K_DataIn_z > 2_000) and (K_DataIn_z <> 0) and (range0 > Ping_dist) and (range1 > Ping_dist) and (range2 > Ping_dist) and (IR_range0 < IR_dist) and (IR_range1 < IR_dist) and (IR_range2 < IR_dist)
       K_forward                      
       waitcnt(clkfreq/100 + cnt)  
        
    if (K_DataIn_z < 700) and (K_DataIn_z <> 0) and (range3 > Ping_dist) and (IR_range3 < IR_dist)
       K_reverse
       waitcnt(clkfreq/100 + cnt)
...
PUB Auto_Mode                            'IR_range(x) gets larger as objects closer. Range(x) gets smaller.                                       
 repeat
    ...
    If  (range0 > Ping_dist) and (range1 > Ping_dist) and (range2 > Ping_dist) and (IR_range0 < IR_dist) and (IR_range1 < IR_dist) and (IR_range2 < IR_dist) and (DataIn == "y")
        waitcnt(clkfreq/2 + cnt)
        EL_tapeCOG := cognew(EL_tape, @Stack2)
        ServoCog := cognew(Servo(front_servo4), @stack1) 
        Forward("y")

    If (range1 < Ping_dist) and (DataIn == "y")  or IR_range0 > IR_dist_ground        'Center front
       waitcnt(clkfreq/2 + cnt)
       BeepCog := cognew(Beep, @Stack3)
       ServoCog := cognew(Servo(rear_servo5), @stack4)
       Reverse(60)
    
    If (range0 < Ping_dist) and (DataIn == "y") or (IR_range1 > IR_dist)                'Right side
       waitcnt(clkfreq/2 + cnt) 
       Rotate90CCW
    
    If (range2 < Ping_dist) and (DataIn == "y")  or (IR_range2 > IR_dist)               'Left
       waitcnt(clkfreq/2 + cnt)
       Rotate90cw
...
PUB Forward(mode)  | Frequency, Pulse_length, t         'AUTO_kinect                    'Repeats until check does not equal 
  ...
  Pulse_length := Frequency/Speed       'This controls the duration within a single cycle of the above frequency that the led stays on. The minumum pulse width is 1/80_000_000. So Pulse_length >= clkfreq
   ...
   'Would like to exit the loop when transmitter power is shut off, but I can't figure out how(Need to exit this loop).
    repeat until dataIn <> mode or range0 < Ping_dist or range1 < Ping_dist or range2 < Ping_dist or IR_range0 > IR_dist_ground or IR_range1 > IR_dist or IR_range2 > IR_dist
...
PUB Reverse(iterations) | Frequency, Pulse_length, t          ''For AUTO MODE.  Remember:  'Cannot check DataIn := XB.Rx while in the repeat loop. The timing of the loop is important. The time it takes to check XB.Rx throws off the pwm.
  ...
  if (DataIn == "y") or (DataIn == "k") and (range3 > Ping_dist) and IR_range3 < IR_dist
   repeat iterations
    if range3 > Ping_dist and IR_range3 < IR_dist   
      t := cnt                                                                      'The reading of the current clock position must be as close as possible to the repeat loop within which it is used.                                                 
...
PUB K_Forward  | Frequency, Pulse_length, t         'AUTO_kinect                    'Repeats until check does not equal 
  ...
  repeat until (K_DataIn_z < 2_000) OR (K_CharIn == "l") OR (range0 < Ping_dist) OR (range1 < Ping_dist) OR (range2 < Ping_dist) OR (IR_range0 > IR_dist) OR (IR_range1 > IR_dist) OR (IR_range2 > IR_dist)
      t := cnt                                     'The reading of the current clock position must be as close as possible to the repeat loop within which it is used.
      repeat 1_000                                 'Cannot check DataIn := XB.Rx while in the repeat loop. The timing of the loop is important. The time it takes to check XB.Rx throws off the pwm.                           
          phsa := phsb:= -Pulse_length             'Why is this negative?
          t += Frequency                           'phsa is the length of the pulse duration.       
...
PUB K_Reverse | Frequency, Pulse_length, t          ''For AUTO MODE.  Remember:  'Cannot check DataIn := XB.Rx while in the repeat loop. The timing of the loop is important. The time it takes to check XB.Rx throws off the pwm.
  ...
  repeat until (K_DataIn_z > 700) OR (K_CharIn == "l") OR (range3 < Ping_dist) OR (IR_range3 > IR_dist) 
     t := cnt  
...
repeat until (K_DataIn_x > -200) OR (K_CharIn == "l") OR (range0 < Ping_dist) OR (IR_range1 > IR_dist)
  t := cnt                             
  repeat 1_000
     phsa := phsb:= -Pulse_length                                   'The reading of the current clock position must be as close as possible to the repeat loop within which it is used.
     t += Frequency                                                 'Cannot check DataIn := XB.Rx while in the repeat loop. The timing of the loop is important. The time it takes to check XB.Rx throws off the pwm.
...
repeat until (K_DataIn_x < 200) OR (K_CharIn == "l") OR (range2 < Ping_dist) OR (IR_range2 > IR_dist)
  t := cnt                             
  repeat 1_000
     phsa := phsb:= -Pulse_length                                   'The reading of the current clock position must be as close as possible to the repeat loop within which it is used.
     t += Frequency                                                 'Cannot check DataIn := XB.Rx while in the repeat loop. The timing of the loop is important. The time it takes to check XB.Rx throws off the pwm.
