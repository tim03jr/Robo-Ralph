...
''Problems:
  ...
  'Replace the center Ping with an IR sensor because angle doesn't allow for reflection of sound waves back to ping.
...
ping_pin3 = 3   'Rear right(From it's point of view) 
ping_pin4 = 4   'Rear center
ping_pin5 = 5   'Rear left(From it's point of view)
'EncoderPin_Right = 7  'It's right
'EncoderPin_Left = 8 'It's left
distance  = 5   'Distance at which things are sensed
...
PUB Servo  | Frequency, Pulse_length, t, i
...
PUB Pings                                                  'Might have to put a wait after each ping reading.
   repeat     
     ...
     range4 := ping.Inches(ping_pin4)      'Back         
     range5 := ping.Inches(ping_pin5)      'Back
...
PUB AutoMode                            'IR_range(x) gets larger as objects closer. Range(x) gets smaller.
 ...
 repeat
    ...
    If range0 < distance and DataIn == "y" or IR_range1 > IR_dist         'If something is sensed on the right
       waitcnt(clkfreq + cnt) 
       Rotate90CCW
     
    If range1 < distance and DataIn == "y"  or IR_range0 > IR_dist_ground  
       waitcnt(clkfreq + cnt)
...
Pulse_length := Frequency/Speed       'This controls the duration within a single cycle of the above frequency that the led stays on. The minumum pulse width is 1/80_000_000. So Pulse_length >= clkfreq
 ...
 'Would like to exit the loop when transmitter power is shut off, but I can't figure out how(Need to exit this loop).
  repeat until DataIn <> "y" or range0 < distance or range1 < distance or range2 < distance 
    t := cnt                                     'The reading of the current clock position must be as close as possible to the repeat loop within which it is used.
...
PUB Speed_Adjust(Pulse_Width) 'Result is returned. It is the new speed to be sent to the motors.           AUTO MODE
if Pulse_Width <> Desired_Width     '<> means not equal to.
  if Pulse_Width > Desired_Width
    New_Width := Pulse_Width - 1    'The := is for setting variables
  if Pulse_Width < Desired_Width
    New_Width := Pulse_Width + 1    'Might need to add more than just one here.
  Result := New_Width


PUB Encoder_Read | begining, end, time_high, state 

                                                   'The input pin has a 1.65V threshold
'Set up counter modules to read pulse width.
'repeat                     
  {ctra[30..26] := %01000                           'Positive detection mode
  ctra[5..0] := EncoderPin_Left
  ctrb[30..26] := %01000
  ctrb[5..0] := EncoderPin_Right
  frqa := 1
  frqb := 1
   phsa~                             'This clears the phs register from it's last reading.
   dira[EncoderPin_Left]~            'This actually starts the reading of the pin.
   phsb~ }
  ' dira[EncoderPin_Right]~  
{
 dira[EncoderPin_Left]~
 repeat
   waitcnt(clkfreq/100 + cnt)   
   if ina[EncoderPin_left]  == 1
     begining := cnt
      
   if ina[EncoderPin_left]  == 0                     'The value for time_high varies widly.
      end := cnt   
      time_high := end - begining 
   pst.char("z") 
   'pst.dec(ina[EncoderPin_left] )
  pst.dec(time_high)   }
