...
Pub Track_Mode                                  'If no skeleton is being tracked, return to start method.
 ...
 repeat
    ...
    if {(knct_dist > track_dist) and }(range0 > distance) and (range1 > distance) and (range2 > distance) and (IR_range0 < IR_dist) and (IR_range1 < IR_dist) and (IR_range2 < IR_dist)
    ...
    if {(knct_dist  < track_dist) and } (DataIn == "k") and (range1 < distance) or IR_range0 > IR_dist_ground        'Rear
      waitcnt(clkfreq/2 + cnt)
      BeepCog := cognew(Beep, @Stack3)
      ServoCog := cognew(Servo(rear_servo5), @stack4) 
      reverse(30)

    If (range0 < distance) and (DataIn == "k") or (IR_range1 > IR_dist)                'Right side
       waitcnt(clkfreq/2 + cnt) 
       Rotate90CCW

    If (range2 < distance) and (DataIn == "k")  or (IR_range2 > IR_dist)               'Left
       waitcnt(clkfreq/2 + cnt)
       Rotate90cw
