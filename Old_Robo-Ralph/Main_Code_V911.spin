'Difference from V8: Getting rid of encoders, adding RGB led for battery charge and kinect mode.
'Adding L/R priority in Kinect mode.
...
speed          = 4   'Smaller is faster. This controls the speed of the wheels. It the time within one cycle that the signal is high. In this case. 1/5th   |
IR_dist        = 1400   'Higher is closer. 1500 is about 5 inches                                                                                           |
IR_dist_ground = 1500 '                                                                                                                                     |
...
k_MoveCOG := cognew(K_Move, @Stack9)
repeat
 ...
 'Seven different scenerios:

 'User is Far and left     (R>L)
  If K_DataIn_x < -150 and K_DataIn_z > K_Far_dist
    Left_Motor_speed  := 0
    Right_Motor_Speed := (K_DataIn_x)
     
 'User is Far and right    (L>R)
  If K_DataIn_x > 150 and K_DataIn_z > K_Far_dist
    Left_Motor_speed  := (K_DataIn_x)
    Right_Motor_Speed := 0
       
 'User is Far and centered   (R=L=0)
  If K_DataIn_x > -150 and K_DataIn_x < 150 and K_DataIn_z > K_Far_dist
    Left_Motor_speed  :=  0
    Right_Motor_Speed :=  0
    
 'User is Near and left       (L = -R)
  If K_DataIn_x < -150 and K_DataIn_z < K_Near_dist
    Left_Motor_speed  := (4000/speed)                               
    Right_Motor_Speed := 0
       
 'User is Near and right        (R = -L)
  If K_DataIn_x > 150 and K_DataIn_z < K_Near_dist
    Left_Motor_speed  := 0
    Right_Motor_Speed := (4000/speed) 
     
 'User is Near and centered      (R=L=PulseLength)
 If K_DataIn_x > -150 and K_DataIn_x < 150 and K_DataIn_z < K_Near_dist
    Left_Motor_speed  :=  (4000/speed)
    Right_Motor_Speed :=  (4000/speed)                               'This is to make the wheels stop.
...
PUB K_Move  | Frequency, Pulse_length, t         'AUTO_kinect                    'Repeats until check does not equal
  ...
  Frequency := clkfreq/20_000           ' Frequency = 4000 clk cycles        This controls the frequency that the repeat loops cycles
  Pulse_length := Frequency/Speed       ' Pulse_length = 800                 This controls the duration within a single cycle of the above frequency that the led stays on. The minumum pulse width is 1/80_000_000. So Pulse_length >= clkfreq

  repeat until (DataIn <> "k") OR (K_DataIn_z < K_Near_dist) OR (K_DataIn_z < K_Far_dist) OR (K_CharIn == "l") OR (K_DataIn_x < -K_Horiz_dist) OR (K_DataIn_x > K_Horiz_dist) OR (range0 < Ping_dist) OR (range1 < Ping_dist) OR (range2 < Ping_dist) OR (IR_range0 > IR_dist) OR (IR_range1 > IR_dist) OR (IR_range2 > IR_dist)
      ...
      repeat 10                                 'Cannot check DataIn := XB.Rx while in the repeat loop. The timing of the loop is important. The time it takes to check XB.Rx throws off the pwm.                           
          phsa := -Pulse_length + Left_Motor_Speed
          phsb := -Pulse_length + Right_Motor_Speed            'Why is this negative?
...
if DataIn == "y" ' or DataIn == "k"    
    t := cnt                                                                      'The reading of the current clock position must be as close as possible to the repeat loop within which it is used.                                                 
    repeat rotate_time                                                                'The number here represents the number of times the repeat loop repeats.                           
        phsa := phsb:= -Pulse_length                                 
        t += Frequency                                                            'phsa is the length of the pulse duration.       
        waitcnt(t)
